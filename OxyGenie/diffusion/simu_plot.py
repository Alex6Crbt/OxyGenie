...
class Simu_plot:
    ...
    def champ_vect(self, params, L_result, i, s_ech=5):
        ...
        if i < 0:
            ...
            plt.title(
                f"Champ vectoriel des flux de concentration à T = {(params.nt-i)*params.dt:0.1e}s")
        else:
            # Gradients temporels
            dC_dy, dC_dx = np.gradient(-L_result[i] + L_result[i + 1])
            plt.title(
                f"Champ vectoriel des flux de concentration à T = {i*params.dt:0.1e}s")
        ...
        X, Y = np.linspace(0, params.Lx, params.nx), np.linspace(
            0, params.Ly, params.ny)
        X, Y = np.meshgrid(X, Y)
        # Sous-échantillonnage des résultats
        if s_ech == 1:
            X, Y, pdC_dx, pdC_dy, pc = X[::s_ech, ::s_ech], Y[::s_ech,
                                                              ::s_ech], dC_dx[::s_ech, ::s_ech], dC_dy[::s_ech, ::s_ech], c[::s_ech, ::s_ech]
            # ,   scale=1e-0/(2*Lx),)
            q = ax.quiver(X, Y, pdC_dx, pdC_dy, pc, cmap='plasma')

        else:
            X, Y, pdC_dx, pdC_dy, pc = X[::s_ech, ::s_ech], Y[::s_ech, ::s_ech], dC_dx[::s_ech, ::s_ech] / \
                c[::s_ech, ::s_ech], dC_dy[::s_ech, ::s_ech] / \
                c[::s_ech, ::s_ech], c[::s_ech, ::s_ech]
            q = ax.quiver(X, Y, pdC_dx, pdC_dy, pc, cmap='plasma',
                          scale=1e-0 / (2 * params.Lx),)
    ...
    def anim(self, params, L_result):
        ...
        line4, = ax4.plot(
            [k * params.dt * params.step for k in range(params.nt // params.step)], Ltc, label="Centre")
        line5, = ax4.plot([k * params.dt * params.step for k in range(
            params.nt // params.step)], Ltfx, label="Bord X Gauche")
        line6, = ax4.plot([k * params.dt * params.step for k in range(
            params.nt // params.step)], Ltfy, label="Bord Y Bas")
        ...
        def update(frame):
            ...
            line1.set_data(
                [k * params.step for k in range(speed * frame + 1)], S[:speed * frame + 1])
            line2.set_data([k * params.dx for k in range(params.nx)],
                           L_result[speed * frame][params.nx // 2, :])
            line3.set_data([k * params.dy for k in range(params.ny)],
                           L_result[speed * frame][:, params.ny // 2])
            line4.set_data(
                [k * params.dt * params.step for k in range(speed * frame)], Ltc[:speed * frame])
            line5.set_data(
                [k * params.dt * params.step for k in range(speed * frame)], Ltfx[:speed * frame])
            line6.set_data(
                [k * params.dt * params.step for k in range(speed * frame)], Ltfy[:speed * frame])
    ...
    def anim_vect(self, params, L_result, s_ech=5):
        ...
        im = ax1.imshow(L_result[len(L_result) // 2], extent=(0, params.Lx,
                                                              0, params.Ly), origin='lower', cmap='viridis')  # , norm=LogNorm())
        fig.colorbar(im, ax=ax1, label="Concentration")

        # Initialisation du champ vectoriel
        dC_dy, dC_dx = np.gradient(
            -L_result[-(nt // params.step) // 4 - 1] + L_result[-(nt // params.step) // 2])
        c = np.sqrt(dC_dx**2 + dC_dy**2) + 1e-10
        ...
        X, Y = np.meshgrid(X, Y)
        X, Y, pdC_dx, pdC_dy, pc = X[::s, ::s], Y[::s,
        ...
        # Stocke (pdC_dx, pdC_dy) pour chaque frame
        anidc = np.empty((nt, *pdC_dx.shape, 2))
        ...
        for i in tqdm(range(nt // params.step)):
            ...
            # np.sign(pdC_dx)*np.log(1+np.abs(pdC_dx/c))
            anidc[i, :, :, 0] = pdC_dx / pc
            # np.sign(pdC_dy)*np.log(1+np.abs(pdC_dy/c))
            anidc[i, :, :, 1] = pdC_dy / pc
            anic[i] = pc
        ...
        def update(frame):
            """Met à jour l'image pour chaque étape de l'animation."""
            im.set_array(L_result[speed * frame])
            dC_dy, dC_dx = anidc[speed * frame, :,
                                 :, 1], anidc[speed * frame, :, :, 0]
            pc = anic[speed * frame]
